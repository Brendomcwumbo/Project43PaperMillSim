...
# Default rail stops from your original gantry rail behavior.
_DEFAULT_RAIL_POSITIONS = [
    Gf.Vec3d(-237.0, -160.0, 561.0),  # J
    Gf.Vec3d(-237.0, -80.0, 561.0),   # K
    Gf.Vec3d(-237.0, 45.0, 561.0),    # L
    Gf.Vec3d(-237.0, 140.0, 561.0),   # ;
]
...
def _resolve_positions(db):
    ...
    if not use_custom:
        return _DEFAULT_RAIL_POSITIONS

    return [
        _as_vec3(_get_input(db, "positionJ", _DEFAULT_RAIL_POSITIONS[0]), _DEFAULT_RAIL_POSITIONS[0]),
        _as_vec3(_get_input(db, "positionK", _DEFAULT_RAIL_POSITIONS[1]), _DEFAULT_RAIL_POSITIONS[1]),
        _as_vec3(_get_input(db, "positionL", _DEFAULT_RAIL_POSITIONS[2]), _DEFAULT_RAIL_POSITIONS[2]),
        _as_vec3(_get_input(db, "positionSemicolon", _DEFAULT_RAIL_POSITIONS[3]), _DEFAULT_RAIL_POSITIONS[3]),
    ]
...
def _on_keyboard_event(event, *args, **kwargs):
    ...
    if _STATE["desired_index"] != _STATE["current_index"]:
        ...
        carb.log_info(
            f"[scriptnode_gantry_rail] key press -> desired_index={_STATE['desired_index']} "
...
def setup(db):
    ...
    _STATE["move_speed"] = float(_get_input(db, "moveSpeed", 30.0))
    ...
    db.log_warning("[scriptnode_gantry_rail] setup() executed. Waiting for compute ticks.")
...
def compute(db):
    ...
    if not _STATE["did_log_first_compute"]:
        db.log_warning("[scriptnode_gantry_rail] compute() is running.")
    ...
    if debug_heartbeat and (now - _STATE["last_heartbeat"]) >= 1.0:
        carb.log_info(
            f"[scriptnode_gantry_rail] heartbeat: current_index={_STATE['current_index']} "
    ...
    prim, resolved_path = _resolve_target_prim(db, stage, "/World/Gantry")
    if prim is None:
        if not _STATE["did_warn_no_prim"]:
            db.log_warning(
                f"No valid Xformable target prim found. primPath='{resolved_path}'. "
                "Set db.inputs.primPath to your gantry prim path or select a prim in the stage."
            )
